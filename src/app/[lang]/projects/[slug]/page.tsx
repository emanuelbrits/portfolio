...
export default async function ProjectPage({ params }: { params: { slug: string, lang: string } }) {
    ...
    const projects: Record<string, Project> = {
        ...
        "andrade-bardawil": {
            ...
            year: "2025",
            imgs: ["/images/andradeBardawil1.jpg", "/images/andradeBardawil2.jpg", "/images/andradeBardawil3.jpg"]
        },
        "oikos-pousada": {
            title: "Oikos Pousada",
    ...
    return (
        <main className="w-screen overflow-x-hidden px-6 lg:px-7.5 xl:px-14 2xl:px-60">
            ...
            <div className="flex flex-col mt-32">
                <h1 className="text-5xl md:text-6xl xl:text-8xl font-bold mb-4">{project.title}<span className='text-red-500 !text-5xl md:!text-6xl xl:!text-8xl !font-bold'>.</span></h1>
                ...
                <p className="text-neutral-700 text-lg md:text-xl lg:text-2xl xl:text-3xl my-12 border-l-5 md:border-l-10 pl-3 border-red-500">{project.description}</p>
                    ...
                    {project.imgs && project.imgs.length > 0 && (
                        <div className="flex flex-col md:col-span-3 lg:col-span-4 xl:col-span-5 gap-6">
                            {project.imgs.map((imgSrc, index) => (
                                <div
                                    key={index}
                                    className="relative w-full h-64 lg:h-124 xl:h-148 2xl:h-186 rounded-3xl overflow-hidden"
                                >
                                    <Image
                                        src={imgSrc}
                                        alt={project.title}
